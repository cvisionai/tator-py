...
def test_media_change_log(host, token, project, attribute_video_type):
    ...
    # Create the media.
    media_ids = [
        tator_api.create_media_list(project=project, create_media_list_request=[media_spec]).id[0]
...
def test_leaf_type_change_log(host, token, project, leaf_type):
    ...
    response = tator_api.create_leaf_list(project=project, create_leaf_list_request=leaf_specs)
...
def test_change_log_util(host, token, project, video_type):
    ...
    # Create the media.
    media_id = tator_api.create_media_list(

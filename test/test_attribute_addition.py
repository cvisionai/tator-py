...
def test_add_same_attribute_with_different_dtypes(
    ...
    # Adding the same attribute with a different `dtype` should not raise an exception
    addition["addition"]["dtype"] = "string"
    response = tator_api.add_attribute(id=attribute_box_type, attribute_type_spec=addition)

    assert f"New attribute type '{new_attr_name}' added" == response.message
